...
describe("DeviceTreeManager Class", function () {
    ...
    it("Tree height is 2, and creating n merkle proof from the same device", async () => {
        let devTreeManager: DeviceTreeManager = new DeviceTreeManager(8, 1)
        ...
        const fileName = "zkREC_v2_input.json"
        const filePath = `${folderPath}/${fileName}`

        fs.writeFileSync(filePath, result)
    })

    xit("Create n different devices and signing message, then insert to merkle tree", () => {
        ...
        for (let deviceId = 0; deviceId < 4; deviceId++) {
            ...
            let signature = devTreeManager.deviceAttest(deviceId, 100n)
